...
namespace MiniProject.Controllers
{
    ...
    public class ProductController : ControllerBase
    {
        ...
        [HttpPost]
        [Route("AddProduct")]
        public async Task<IActionResult> Post([FromBody][Bind(include: "name,price,imageUrl,cid")] Product product)
        {
            try
            {
        ...
        public IActionResult UploadImage()
        {
            try
            {
                ...
                var filePath = Path.Combine(@"C:\AngularWorkSpace\MiniProject\src\assets\images\", fileName);

                using (var stream = new FileStream(filePath, FileMode.Create))
                {
                    file.CopyTo(stream);
                }
                string url = @"\assets\images\" + fileName;
